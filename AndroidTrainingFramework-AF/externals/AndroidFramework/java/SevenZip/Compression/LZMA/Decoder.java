...
public class Decoder
{
	...
	/**
	 * It resets the window size to the outsize of a file, if the file's size is smaller
	 */
	public boolean SetDecoderProperties(byte[] properties, long outSize)
	{
		if (properties.length < 5)
			return false;
		int val = properties[0] & 0xFF;
		...
		if (outSize < dictionarySize)
			return SetDictionarySize((int)outSize);
		else
			return SetDictionarySize(dictionarySize);
	}
}
