...
public class GMPHelper extends IABTransaction
{
	...
	{
		INFO(TAG, "Consuming product with Google from Game");
		JDUMP(TAG, value);
	...
	public void sendOrderIdConfirmation(final String orderId)
	{
		...
		sb.append(reqHandler.encodeString(orderId));
			...
			public void run ()
			{
				...
				confirm_invalid_store_dialog.setButton(IABDialog.BUTTON_NEGATIVE,R.string.iab_cancel, new View.OnClickListener() {
					public void onClick(View v) {
						...
						//Error code
						int code =  (InAppBilling.mMKTStatusHk == InAppBilling.MKT_FREEDOM_RUNNING)?GMPUtils.IABHELPER_FREEDOM_DETECTED:GMPUtils.IABHELPER_USER_CANCELLED;
						String message =  utils.getResponseDesc(code);
				...
				try{
					...
					if (!TextUtils.isEmpty(mItemId))
					{
						...
						if (!TextUtils.isEmpty(managed) && managed.equals("1")) 
						{
							...
							INFO(TAG, "Store Cert: " + purchase.toString());
							bundle.clear();
							...
							// bundle.putByteArray(InAppBilling.GET_STR_CONST(IAB_TP_SKU),(purchase.getSku()!=null)?purchase.getSku().getBytes():emptyarray);
							...
							bundle.putByteArray(InAppBilling.GET_STR_CONST(IAB_SHOP),(InAppBilling.GET_STR_CONST(IAB_GOOGLEPLAY).getBytes()));
							bundle.putByteArray(InAppBilling.GET_STR_CONST(IAB_STORE_CERT), purchase.toString().getBytes() );
	...
	public String getJSONHeaders(Device device)
	{
		...
		try
		{
			...
			jObj.put("Accept", InAppBilling.GET_STR_CONST(IAB_ECOM_API_VERSION_13));//"application/com.gameloft.ecomm.android.iap-v1.3+plain"
		#else
			jObj.put("Accept", InAppBilling.GET_STR_CONST(IAB_ECOM_API_VERSION));//"application/com.gameloft.ecomm.android.iap-v1.2+plain"
		#endif
	...
	public void sendIABNotification(final PurchaseInfo purchase)//Send Gameloft Notification
	{
		...
		sb.append(InAppBilling.GET_STR_CONST(IAB_CONFIRM_ITEM_DELIVERED));
			...
			{
				sb.append("&injected_igp="+SUtils.getInjectedIGP());
			}
			if (!TextUtils.isEmpty(SUtils.getInjectedSerialKey()))
			{
				sb.append("&d="+SUtils.getInjectedSerialKey());
			}
		#endif
