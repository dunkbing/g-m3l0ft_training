...
public class GMPActivity extends Activity
{
    ...
    private void performPurchaseRequest() 
	{
		...
		String sku = mServerInfo.getItemUID();
	...
	IABCallBack mPurchaseFinishedCB = new IABCallBack()
	{
		public void runCallBack(Bundle bundle)
		{
			...
			else if (result.isFailure()) 
			{
				...
				//Error code
				GMPUtils utils = GMPUtils.getInstance();
				String message =  utils.getResponseDesc(result.mResponse);
				bundle.putByteArray(InAppBilling.GET_STR_CONST(IAB_TP_ERROR),(message!=null)?message.getBytes():null);				
				bundle.putInt(InAppBilling.GET_STR_CONST(IAB_TP_ERROR_CODE), result.mResponse);
			...
			else
			{
				try{
					...
					else
					{
						...
						//Error code
						GMPUtils utils = GMPUtils.getInstance();
						String message =  utils.getResponseDesc(GMPUtils.IABHELPER_FREEDOM_DETECTED);
						bundle.putByteArray(InAppBilling.GET_STR_CONST(IAB_TP_ERROR),(message!=null)?message.getBytes():null);				
						bundle.putInt(InAppBilling.GET_STR_CONST(IAB_TP_ERROR_CODE), GMPUtils.IABHELPER_FREEDOM_DETECTED);
