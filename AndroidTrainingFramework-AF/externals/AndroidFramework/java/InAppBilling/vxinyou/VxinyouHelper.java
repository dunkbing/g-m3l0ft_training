...
import APP_PACKAGE.iab.VxinyouIABActivity;

public class VxinyouHelper extends IABTransaction
{	
	private static String mCurrentId 		= null;
	private static String mLoginAuthCode 	= null;
	private static String mLoginAccessToken	= null;
	private static String mPurchaseId 		= null;
	private static String mGamePrice 		= null;
	
	private static String mUrlToken 		= null;
	private static String mUrlUerInfo 		= null;
	private static String mUrlOrder 		= null;
	private static String mUrlCheck 		= null;
	
	private static long mLoginTime 			= 0;
	private static boolean mOpResult	 	= false;
	private static Device mDevice;

	private static final int JSON_HEADERS_GET_TOKEN_URL 	= 0;
	private static final int JSON_HEADERS_USER_INFO_URL 	= 1;
	private static final int JSON_HEADERS_ORDER_URL 		= 2;
	private static final int JSON_HEADERS_CHECK_URL 		= 3;
	
	interface VxinyouHelperConst
	{
		public static final String STR_APP_ID 	= VXINYOU_APPID; 	//must be defined in config.bat
		public static final String STR_APP_KEY 	= VXINYOU_APPKEY; 	//must be defined in config.bat
	}

	public VxinyouHelper(ServerInfo si)
	{
		super(si);
	...
	@Override
	public boolean restoreTransactions(){ERR(TAG, "XINYOU API does not support restoreTransactions!");return true;};
	...
	public boolean requestTransaction(String id)
	{
		...
		try{
			mCurrentId = id;
			mUrlToken = mServerInfo.getBillingAttribute(mCurrentId,"API_get_token");
			mUrlUerInfo = mServerInfo.getBillingAttribute(mCurrentId,"API_get_user_infor");
			mUrlOrder = mServerInfo.getBillingAttribute(mCurrentId,"url");
			mUrlCheck = mServerInfo.getBillingAttribute(mCurrentId,"url_check");
			mGamePrice = mServerInfo.getGamePrice();
			
			if(!TextUtils.isEmpty(VxinyouHelperConst.STR_APP_ID) && !TextUtils.isEmpty(VxinyouHelperConst.STR_APP_KEY) && mCurrentId != null && mGamePrice != null)
			{
				DBG(TAG, "Item: " + mCurrentId + " Price: " + mGamePrice);
				Intent intent = new Intent(SUtils.getContext(), VxinyouIABActivity.class);
				intent.putExtra("PRICE", mGamePrice);
				SUtils.getContext().startActivity(intent);
			}
			else
			{
				ERR(TAG,"Unable to initiate purchase!");
	...
	static void GetAccessToken(String authorization, long time)
	{
		INFO(TAG, "[GetAccessToken]");
		
		try{
			mLoginAuthCode = authorization;
			mLoginTime = time;
			String jsonHeaders = getJsonHeaders(mDevice, JSON_HEADERS_GET_TOKEN_URL);
			
			if(!TextUtils.isEmpty(jsonHeaders))
			{
				DBG(TAG, "[GetAccessToken] url: " + mUrlToken);
				DBG(TAG, "[GetAccessToken] jsonHeaders: " + jsonHeaders);
				
				IABRequestHandler reqHandler = IABRequestHandler.getInstance();
				reqHandler.doRequestByGet(mUrlToken, null, jsonHeaders, new IABCallBack()
				{
					public void runCallBack(Bundle bundle)
					{
						int result = bundle.getInt(IABRequestHandler.KEY_REQUEST_RESULT);
						DBG(TAG, "[GetAccessToken] result: " + result);
						
						if (result == IABRequestHandler.SUCCESS_RESULT)
						{
							String response = bundle.getString(IABRequestHandler.KEY_RESPONSE);
							DBG(TAG, "[GetAccessToken] response: " + response);
							
							try{
								JSONObject jObject = new JSONObject(response);
								String accessToken = jObject.getJSONObject("data").getString("code");
								DBG(TAG, "[GetAccessToken] accessToken: " + accessToken);
	...
	{
		INFO(TAG, "[GetUserInfo]");
		
		try{
			mLoginAccessToken = accessToken;
			String jsonHeaders = getJsonHeaders(mDevice, JSON_HEADERS_USER_INFO_URL);
			
			if(!TextUtils.isEmpty(jsonHeaders))
			{	
				DBG(TAG, "[GetUserInfo] url: " + mUrlUerInfo);
				DBG(TAG, "[GetUserInfo] jsonHeaders: " + jsonHeaders);
				
				IABRequestHandler reqHandler = IABRequestHandler.getInstance();
				reqHandler.doRequestByGet(mUrlUerInfo, null, jsonHeaders, new IABCallBack()
				{
					public void runCallBack(Bundle bundle)
					{
						int result = bundle.getInt(IABRequestHandler.KEY_REQUEST_RESULT);
						DBG(TAG, "[GetUserInfo] result: " + result);
						
						if (result == IABRequestHandler.SUCCESS_RESULT)
						{
							String response = bundle.getString(IABRequestHandler.KEY_RESPONSE);
							DBG(TAG, "[GetUserInfo] response: " + response);
							
							try{
								JSONObject jObject = new JSONObject(response);
								String userId = jObject.getJSONObject("data").getString("id");
								String appuserId = jObject.getJSONObject("data").getString("name");
								DBG(TAG, "[GetUserInfo] userId: " + userId + " appuserId: " + appuserId);
								
								if(userId != null && appuserId != null)
									OrderRequest(userId, appuserId);
								else
									IABResultCallBack(IAB_BUY_FAIL);
	...
	private static void OrderRequest(String userId, String appuserId)
	{
		...
		try{
			...
			String qry = "&userid="+userId+
						"&appid="+VxinyouHelperConst.STR_APP_ID+
						"&appuserid="+appuserId+
						"&accesstoken="+mLoginAccessToken;
			
			if(!TextUtils.isEmpty(jsonHeaders))
			{
				DBG(TAG, "[OrderRequest] url: " + mUrlOrder);
				DBG(TAG, "[OrderRequest] qry: " + qry);
				...
				reqHandler.doRequestByGet(mUrlOrder, qry, jsonHeaders, new IABCallBack()
				{
					public void runCallBack(Bundle bundle)
					{
						...
						if (result == IABRequestHandler.SUCCESS_RESULT)
						{
							...
							try{
								JSONObject jObject = new JSONObject(response);
								String billno = jObject.getJSONObject("data").getString("billno");
								mPurchaseId = jObject.getJSONObject("data").getString("correlationno");
								DBG(TAG, "[OrderRequest] billno: " + billno + " purchase ID: " + mPurchaseId);
								
								if(billno!= null && mPurchaseId != null)
									VxinyouIABActivity.startPayment(billno);
								else
									IABResultCallBack(IAB_BUY_FAIL);
	...
	private static String getJsonHeaders(Device device, int type)
	{
		...
		try{
			if(type == JSON_HEADERS_GET_TOKEN_URL)
			{
				jObj.put("x-up-gl-appid", VxinyouHelperConst.STR_APP_ID);
				jObj.put("x-up-gl-authorization", mLoginAuthCode);
				jObj.put("x-up-gl-time", mLoginTime);
				jsonHeaders = jObj.toString();
			}
			else if(type == JSON_HEADERS_USER_INFO_URL)
			{
				jObj.put("x-up-gl-appid", VxinyouHelperConst.STR_APP_ID);
				jObj.put("x-up-gl-token", mLoginAccessToken);
				jObj.put("x-up-gl-time", mLoginTime);
				jsonHeaders = jObj.toString();
			}
			else if(type == JSON_HEADERS_ORDER_URL)
			{
				...
				jObj.put("x-up-gl-contentid",mCurrentId);
				...
				jObj.put("x-up-gl-price", mGamePrice);
			...
			else if(type == JSON_HEADERS_CHECK_URL)
			{
				...
				String value = mPurchaseId + "_" + VxinyouHelperConst.STR_APP_ID + "_gameloft_china_xinyou";
	...
	static void IABResultCallBack(int IABResult)
	{
		...
		{
			//Payment success at XINYOU, must call Gameloft URL Check and deliver item
			DBG(TAG, "XINYOU Payment Success! Item: " + InAppBilling.mItemID);
			...
			reqHandler.doRequestByGet(mUrlCheck, null, jsonHeaders, new IABCallBack()
			{
				public void runCallBack(Bundle cBbundle)
				{
					...
					if (result == IABRequestHandler.SUCCESS_RESULT)
					{
						...
						{
							ERR(TAG,"XINYOU payment success but failed on eComm: [" + response + "]");
							LOGGING_LOG_INFO(IAP_LOG_TYPE_ERROR, IAP_LOG_STATUS_ERROR, "XINYOU Payment success but url_check failed:" + response);
						}
					}
					else
					{
						ERR(TAG,"XINYOU Payment success but url_check failed!");
						LOGGING_LOG_INFO(IAP_LOG_TYPE_ERROR, IAP_LOG_STATUS_ERROR, "XINYOU Payment success but url_check failed");
					}
		...
		{
			ERR(TAG,"XINYOU Payment Failed!");
			LOGGING_LOG_INFO(IAP_LOG_TYPE_ERROR, IAP_LOG_STATUS_ERROR, "XINYOU Payment Failed!");
			...
			try{
				...
				if(VxinyouIABActivity.mInstance != null)
					VxinyouIABActivity.mInstance.finish();
			}
			catch(Exception ex ){ERR(TAG,"Error invoking reflex method "+ex.getMessage());}
		}
	}
	...
	private void sendOrderIdConfirmation(final String orderId)
	{
		...
		sb.append("18"); //XINYOU is portalID 18
