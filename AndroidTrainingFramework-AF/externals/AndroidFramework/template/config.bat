...
set VERSION_MICRO=0
set VERSION_NANO=0
set VERSION=%VERSION_MAJOR%%VERSION_MINOR%%VERSION_BUILD%%VERSION_MICRO%%VERSION_NANO%
...
REM set USE_GOOGLE_TV_BUILD=0

REM set USE_ANDROID_TV_IGPCODE=0
REM set GGC_GAME_CODE_TV=
...
set ADS_USE_ADCOLONY=0
	...
	set ADCOLONY_SERVERSIDE_REWARDS=1
rem Flurry config
...
set IN_APP_PURCHASE_LIB_CRM_PATH=%~dp0..\..\libs\inapp_purchase
	...
	set GAMELOFT_SHOP=0
		...
		if "%GAMELOFT_SHOP%"=="1" set ENABLE_IAP_PSMS_BILLING=1
	...
	set BAZAAR_STORE=0
	if "%BAZAAR_STORE%"=="1" set AMP_EPK=
	set YANDEX_STORE=0
	if "%YANDEX_STORE%"=="1" set AMP_EPK=
	...
	set ZTE_STORE=0
		set ZTE_PARTNERID=0
	set ATET_STORE=0
	rem ask submissions for these values
	set ATET_APPID=
	set ATET_APPKEY=
...
set /A NEEDSTOREMOVEPERMISSION=%USE_MARKET_INSTALLER%%GOOGLE_MARKET_DOWNLOAD%%GOOGLE_STORE%%GOOGLE_STORE_V3%%AMAZON_STORE%
if "%HDIDFV_UPDATE%"=="2" (
	IF %NEEDSTOREMOVEPERMISSION% GTR 0 (
		set REMOVE_READ_PHONE_STATE_PERMISSION=1
	)
)
...
if "%GC_LETTER%"=="" (
	set APP_NAME=%GAME_NAME%_update%UPDATE_NUM%_%OPERATOR%_%PHONE%_%GL_PRODUCT_ID%_%GGI%_%VERSION_MAJOR%%VERSION_MINOR%%VERSION_BUILD%
) else (
	set APP_NAME=%GAME_NAME%_update%UPDATE_NUM%_%OPERATOR%_%PHONE%_%GL_PRODUCT_ID%_%GGI%_%VERSION_MAJOR%%VERSION_MINOR%%VERSION_BUILD%%GC_LETTER%
)
...
rem * ATET Tv Store			*	atet_tv_store			*
rem * Huawei Store			*	huawei_tv				*
rem *****************************************************
...
rem *****************************************************************************************
rem *** Very important MARK ************ Do not remove **************************************
rem *****************************************************************************************
rem *****************************************************************************************
rem *****************************************************************************************
rem *****************************************************************************************
rem *****************************************************************************************
rem *** The settings bellow these flags are written (added or removed) in: ******************
rem *** AndroidFrameworkConfig\configs\generated\buildConfigH.bat ***************************
rem *** by the BuildConfigurator. ***********************************************************
rem *****************************************************************************************
rem *** You need to use the BuildConfigurator ***********************************************
rem *** ONLY if you ADD or REMOVE the flags bellow this MARK (aka beginGameSpecific) ********
rem *****************************************************************************************
rem *** The buildConfigH.bat will add these flags to config_Android.h header ****************
rem *** when you run the build. *************************************************************
rem *****************************************************************************************
rem *****************************************************************************************
