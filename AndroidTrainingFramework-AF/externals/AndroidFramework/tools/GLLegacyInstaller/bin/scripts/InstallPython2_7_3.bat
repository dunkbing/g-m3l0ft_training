...
:main
	...
	call :uninstall_Python2_7_3
	echo Install path = %installPath% 
	set PythonArchive=python-2.7.3.msi
	...
	if %_OS_BIT% EQU 64 (
		...
		ECHO ______________________________________________________
		call %~dp0wget.exe --quiet -O %temp%\%PythonArchive% http://www.python.org/ftp/python/2.7.3/python-2.7.3.amd64.msi
	) 
	if %_OS_BIT% EQU 32 ( 
		COLOR 0E
		ECHO ______________________________________________________
		ECHO Downloading python package, please wait for 1 minute...
		ECHO ______________________________________________________
		call %~dp0wget.exe --quiet -O %temp%\%PythonArchive% http://www.python.org/ftp/python/2.7.3/python-2.7.3.msi
	)
	COLOR 0F
	if %ERRORLEVEL% == 1 ( 
		@echo off
...
:uninstall_Python2_7_3
	msiexec /qn /norestart /x {C0E31BCC-56FB-42a7-8766-D29E1BD74C7d}
exit /b 0
