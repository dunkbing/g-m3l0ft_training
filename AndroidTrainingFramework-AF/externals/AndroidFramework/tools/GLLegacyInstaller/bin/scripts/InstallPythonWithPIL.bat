...
:main
	...
	svn export https://svn01/vc/gllegacy/release_v2.0/trunk/tools/sdk/PIL-1.1.7.win32-py2.7.exe %temp%\pilkit.exe --force

	COLOR 0F
	if errorlevel 1 ( 
		@echo off
		start %comspec% /c "mode 60,10&title ERROR InstallJava.Bat:&color cf&echo;&echo; Fetching PIL package from the net failed. &echo; For unknown reasons. Retry this setup at a later time. &echo; Otherwise install the jdk manually by going to www.oracle.com and download jdk 6u39 for your platform. &echo; Press a key!&pause>NUL"		
		
		exit /b 1
	) else (
		ECHO PIL is ready to install.
		call %temp%\pilkit.exe INSTALLDIR=%installPath%
		if errorlevel 1 ( 
			ECHO ERROR occured!!!
			pause
		)
	)
	call :delete_pilkit.exe
