...
:get_setup_exe
	...
	echo Calling %~dp0wget.exe with value "%_ROOTDIR%\setup.exe" 
    call %~dp0wget.exe  --quiet -O "%_ROOTDIR%\setup.exe" http://cygwin.com/setup.exe
	if ERRORLEVEL 1 ( 
		@echo off
		start %comspec% /c "mode 60,10&title ERROR InstallCygwin.Bat:&color cf&echo;&echo; Cannot fetch cygwin setup.exe from the net. &echo; Press a key!&pause>NUL"
		EXIT /B 1
	)
goto :eof
...
:setup
    rem Run setup.exe.
    echo  Running setup.exe for updates
	COLOR 08
	robocopy %~dp0 %_ROOTDIR% setup.exe
    "%_ROOTDIR%\setup.exe" --site %_SITE% --quiet-mode --no-shortcuts --root "%_ROOTDIR%" --local-package-dir "%_ROOTDIR%\LocalPackageDir"
    echo  Running setup.exe to install standard package set
...
:config_lsa
	...
	start "" "%_ROOTDIR%\bin\bash.exe" --login -i
exit /B 0
...
:check_for_setup_file
	if NOT exist %~dp0setup.exe ( 
		@echo off	
		start %comspec% /c "mode 60,10&title ERROR InstallCygwin.Bat &color cf&echo;&echo; Cygwin steup.exe does not exists in the same directory as this batch InstallCygwin.Bat. &echo; Please aquire it before running this batch. &echo; Press a key!&pause>NUL"			
		exit /B 1		
	) 
	SET CygwinSetup=%~dp0setup.exe
exit /b 0
...
:main
    ...
    rem call :get_setup_exe
	call :check_for_setup_file
    call :setup
	call :config_lsa
