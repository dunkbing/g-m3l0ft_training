...
void VideoDriver::Init(HWND hWnd)
{
	...
	m_Pen = new Gdiplus::Pen(Color(255, 255, 0));
...
void VideoDriver::CreateInstance()
{
	if (s_Instance == NULL)
	{
...
void VideoDriver::DestroyInstance()
{
	delete s_Instance;
}
...
void VideoDriver::DrawCircle(int cx, int cy, int radius)
{
	// DRAW ELLIPSE
	g_MemoryGraphic->DrawEllipse(m_Pen, cx - radius, cy - radius, radius * 2, radius * 2);
};
...
void VideoDriver::DrawLine(int x1, int y1, int x2, int y2)
{
	/* ----------    LINE    ---------- */
